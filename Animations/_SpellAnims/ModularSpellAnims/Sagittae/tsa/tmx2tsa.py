...
parser.add_argument('-p','--paletteID',default=0,type=int,help='Palette ID for TSA. Default is 1.')
parser.add_argument('-c','--compress',help='Filepath to compress.exe for lz77 compression.')
parser.add_argument('-m','--mode',default=1,help='0=spell,1=box')
args = parser.parse_args()

tsa = [] # List of shorts to write.
class TSA:
    def __init__(self,width,height,tiles,paletteID,mode): # List of bytes.
        self.width = width # Width is the length per list.
        self.height = height
        self.tiles = [ tiles[i:i+self.width] for i in range(0,self.width*self.height,self.width) ] 
        self.mode = mode
        if int(mode) != 0:
            # This sorts the rows in reverse order.
            self.tiles.reverse()
...
if __name__ == '__main__':
    ...
    tsa = TSA(map.width,map.height,map.layers[0].tiles,args.paletteID,args.mode)   
